# Slide 1: Bem-vindos à Empreendedorismo
```mermaid
mindmap
  root((Tipos de<br/>Inovação))
    Desmistificar
    Aplicar na prática
    Gerar valor
```
---

# Slide 2: Invenção vs. Inovação
```mermaid
flowchart LR
    A[Ideia Nova] -->|Fica no papel| B(INVENÇÃO)
    A -->|Aplicada no mercado| C(INOVAÇÃO)
    C --> D[Gera Valor Real]
```
---

# Slide 3: Por que Inovar?
```mermaid
flowchart TD
    A[Mercado Dinâmico] --> B[Consumidores mudam]
    B --> C[Concorrência avança]
    C --> D{Parar no tempo?}
    D -->|Sim| E[Obsolescência]
    D -->|Não| F[Sobrevivência e Crescimento]
```

---

# Slide 4: Os 4 Pilares da Inovação
```mermaid
mindmap
  root((Inovação))
    Produto/Serviço
    Processo
    Marketing
    Organização
```

---

# Slide 5: Inovação em Produto/Serviço
```mermaid
flowchart LR
    A[Produto Atual] --> B{Mudança}
    B -->|Funcionalidade| C[Novo Recurso]
    B -->|Design| D[Nova Usabilidade]
    B -->|Material| E[Novo Componente]
```

---

# Slide 6: Inovação em Processo
```mermaid
flowchart LR
    subgraph ANTES
    A[Estoque Manual] --> B[Atendimento Lento]
    end
    subgraph DEPOIS
    C[Sistema em Nuvem] --> D[Atendimento Ágil]
    end
    ANTES ==>|Automação| DEPOIS
```

---

# Slide 7: Inovação em Marketing
```mermaid
flowchart TD
    A[Produto: Água Mineral] --> B{Como vender?}
    B -->|Tradicional| C[Garrafão avulso]
    B -->|Inovador| D[Assinatura mensal<br/>Garrafa de vidro<br/>Design minimalista]
```

---

# Slide 8: Inovação Organizacional
```mermaid
flowchart TD
    A[Estrutura Tradicional] -->|Hierarquia rígida| B[Chefes e subordinados]
    C[Estrutura Inovadora] -->|Gestão horizontal| D[Líderes rotativos<br/>20% tempo livre<br/>Foco em talentos]
```

---

# Slide 9: Grau de Inovação
```mermaid
flowchart LR
    subgraph Incremental
    A[Evolução] --> B[Melhoria contínua]
    B --> C[Câmera melhor no celular]
    end
    subgraph Radical
    D[Ruptura] --> E[Novo mercado]
    E --> F[Apps de mobilidade]
    end
```

---

# Slide 10: A Combinação dos Tipos
```mermaid
flowchart TD
    A[Novo Produto] --> B[Exige Novo Processo]
    A --> C[Exige Novo Marketing]
    B --> D[Orquestração]
    C --> D
    D --> E[Sucesso do Negócio]
```

---

# Slide 11: Conclusão e Próximos Passos
```mermaid
mindmap
  root((Próximos<br/>Passos))
    Jornada contínua
    Olhar analítico
    Curiosidade ativa
    Protagonizar a mudança
```
